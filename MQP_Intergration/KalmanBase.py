...
def Dynamic_model(x, u, I, Constants):
    ...
    R_B_A_1 = np.array([[np.cos(psi_1) * np.cos(theta_1) - np.sin(phi_1) * np.sin(psi_1) * np.sin(theta_1),
                         -np.cos(phi_1) * np.sin(psi_1),
                         np.cos(psi_1) * np.sin(theta_1) + np.cos(theta_1) * np.sin(phi_1) * np.sin(psi_1)],
                        [np.cos(theta_1) * np.sin(psi_1) + np.cos(psi_1) * np.sin(phi_1) * np.sin(theta_1),
                         np.cos(phi_1) * np.cos(psi_1),
                         np.sin(psi_1) * np.sin(theta_1) - np.cos(psi_1) * np.cos(theta_1) * np.sin(phi_1)],
                        [-np.cos(phi_1) * np.sin(theta_1), np.sin(phi_1), np.cos(phi_1) * np.cos(theta_1)]])
    H312_1 = np.array([[np.cos(theta_1), 0.0, -np.cos(phi_1) * np.sin(theta_1)], [0.0, 1.0, np.sin(phi_1)],
                       [np.sin(theta_1), 0.0, np.cos(phi_1) * np.cos(theta_1)]])
    # odes
    r_dot_1 = vel_1
    vel_dot_1 = 1.0 / Constants[1] * (np.array([[0], [0], [-Constants[1] * 9.8]]) + np.dot(R_B_A_1, np.array(
        [[0], [0], [F_1[0] + F_1[1] + F_1[2] + F_1[3]]])))
    ang_vel_dot_1 = np.dot(np.linalg.inv(I), (np.array(
        [Constants[0] * (F_1[1] - F_1[3]), Constants[0] * (F_1[2] - F_1[0]),
         M_1[0] - M_1[1] + M_1[2] - M_1[3]]))) - np.dot(np.linalg.inv(I), np.transpose(
        np.cross(np.transpose(ang_vel_1), (np.transpose(np.dot(I, ang_vel_1))))))
...
def A_matrix_maker(x, u, constants,
                   I):  # linearized quadcopter dynamical model to be used in P prediction step of Kalman filter
    A = np.zeros((12, 12));
    ...
    A[11, 9] = -np.sin(x[10]) * x[6] * np.sin(x[9]) / (np.cos(x[9]) ** 2.0) + np.cos(x[10]) * x[8] * np.sin(x[9]) / (
        np.cos(x[9]) ** 2);
...
def main():
    ...
    I_Solo = I_Iris = np.array(
        [[114.220782, 0, 0], [0, 212.391757, 0], [0, 0, 274.203654]])  # Moments of intertia around C.O.G Z axis up
    ...
    C_ss = np.zeros((9, 12))
    C_ss[0, 0] = C_ss[1, 1] = C_ss[2, 2] = C_ss[3, 3] = C_ss[4, 4] = C_ss[5, 5] = C_ss[6, 9] = C_ss[7, 10] = C_ss[
        8, 11] = 1
    C_cs = np.zeros((6, 12))
    C_cs[0, 0] = C_cs[1, 1] = C_cs[2, 2] = C_cs[3, 9] = C_cs[4, 10] = C_cs[5, 11] = 1
    # Timing values
    tstep = 0.01  # the time step between each run of the loop
    # t_ss=# time for UAVS self measurement
    # t_cs=#time for UAVS to measure UAVC
    # initializing variables
    u_Solo = u_Iris = u0_Solo;
    xhat_Solo = xhat_SIris = x_Solo = x0_Solo;
    Z_Solo = Z_SIris = x0_Solo;
    # gain generation
    # rise lqr
    ...
    K_1rise = np.array([[-70.710678, 0.000000, 158.113883, -104.015485, -0.000000, 261.412158, 0.000000, -103.544411,
                         8.251852, 0.000000, -485.542917, 0.500000],
                        [0.000000, -70.710678, 158.113883, 0.000000, -104.015485, 261.412158, 103.544411, -0.000000,
                         -8.251852, 485.542917, 0.000000, -0.500000],
                        [70.710678, 0.000000, 158.113883, 104.015485, -0.000000, 261.412158, 0.000000, 103.544411,
                         8.251852, 0.000000, 485.542917, 0.500000],
                        [-0.000000, 70.710678, 158.113883, -0.000000, 104.015485, 261.412158, -103.544411, -0.000000,
                         -8.251852, -485.542917, -0.000000, -0.500000]])
    ...
    K_1hover = np.array([[-70.710678, 0.000000, 158.113883, -107.225913, 0.000000, 273.390320, -0.000000, -104.448166,
                          8.642585, -0.000000, -450.246601, 0.500000],
                         [0.000000, -70.710678, 158.113883, 0.000000, -107.225913, 273.390320, 104.448166, 0.000000,
                          -8.642585, 450.246601, 0.000000, -0.500000],
                         [70.710678, 0.000000, 158.113883, 107.225913, 0.000000, 273.390320, -0.000000, 104.448166,
                          8.642585, -0.000000, 450.246601, 0.500000],
                         [0.000000, 70.710678, 158.113883, 0.000000, 107.225913, 273.390320, -104.448166, 0.000000,
                          -8.642585, -450.246601, 0.000000, -0.500000]])
    ...
    np.array([[-70.710678, -0.000000, 158.113883, -107.987026, -0.000000, 569.001896, 0.000000, -104.660594, 8.732784,
               0.000000, -442.758740, 0.500000],
              [0.000000, -70.710678, 158.113883, -0.000000, -107.987026, 569.001896, 104.660594, -0.000000, -8.732784,
               442.758740, -0.000000, -0.500000],
              [70.710678, -0.000000, 158.113883, 107.987026, -0.000000, 569.001896, 0.000000, 104.660594, 8.732784,
               0.000000, 442.758740, 0.500000],
              [0.000000, 70.710678, 158.113883, -0.000000, 107.987026, 569.001896, -104.660594, -0.000000, -8.732784,
               -442.758740, -0.000000, -0.500000]])
    ...
    K_1 = np.array([[-223.111800, -0.110447, 156.005124, -181.057707, -0.216325, 559.784065, 0.870840, -123.630682,
                     8.728970, 1.696747, -631.685585, 0.508757],
                    [-0.015437, -68.511909, 165.123952, -0.019973, -103.170467, 591.338095, 100.738584, -0.066908,
                     -8.709877, 440.457882, -0.137852, -0.507890],
                    [224.100701, -0.109681, 155.338775, 181.846632, -0.214790, 557.373440, 0.865915, 124.057068,
                     8.689291, 1.685190, 634.311391, 0.506445],
                    [-0.015377, 72.842941, 155.776593, -0.018987, 109.586185, 557.426639, -105.416558, -0.061894,
    ...
    state = 'active'
    gbvar.UAVS = connect('tcp:127.0.0.1:5760', wait_ready=True)
    print "Basic pre-arm checks"
    # Don't try to arm until autopilot is ready
    u_Solo = u0_hover_Solo
    ...
    print gbvar.UAVS.mode
    while (gbvar.UAVS.mode == 'STABILIZE'):
        print "waiting for mode to switch"
        time.sleep(1)
    gbvar.UAVS.armed = True
    while not gbvar.UAVS.armed:
        print "gbvar.UAVS waiting to arm..."
        time.sleep(1)
    ...
    # time.sleep(0.8)
    # print gbvar.UAVS.velocity
    while (state == 'active'):
        # print gbvar.UAVS.last_heartbeat
        # print 'UAVS is armed: %s' %gbvar.UAVS.armed
        gbvar.UAVS.armed = True
        # print 'test0'
        gbvar.UAVS.send_mavlink(msg)
        time.sleep(0.2)
        ...
        p = str(gbvar.UAVS.location.local_frame)
        gbvar.UAVS.flush()
        last_equals = 0
        ...
        # print a
        # print Z_Solo
        # print np.concatenate((position, velocity, attitude), axis=0)
        Z_Solo_new = np.concatenate((position, velocity, attitude), axis=0)
        if np.array_equal(Z_Solo, Z_Solo_new):
            self_measure_received = 'False'
            print 'measurement received= %s' % str(self_measure_received)
        elif np.array_equal(Z_Solo, Z_Solo_new) == False:
        ...
        if self_measure_received == 'True':
            ...
            Ki_ss = np.dot(np.dot(P_ss, np.transpose(C_ss)),
                           (np.linalg.inv(R_ss + np.dot(np.dot(C_ss, P_ss), np.transpose(C_ss)))))
        ...
        xhat_solo_str = str('%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f \n' % (
            xhat_Solo[0], xhat_Solo[1], xhat_Solo[2], xhat_Solo[3], xhat_Solo[4], xhat_Solo[5], xhat_Solo[6],
            xhat_Solo[7],
            xhat_Solo[8], xhat_Solo[9], xhat_Solo[10], xhat_Solo[11]))
...
def main():
    ...
    # gbvar.UAVS = connect('/dev/serial/by-id/usb-3D_Robotics_PX4_FMU_v2.x_0-if00', baud = 115200, rate=6)
    state = 'active'
    # gbvar.UAVS = connect('tcp:127.0.0.1:5763', wait_ready=True)
    print "Basic pre-arm checks"
    # Don't try to arm until autopilot is ready
    u_Solo = u0_hover_Solo
    while not gbvar.UAVS.is_armable:
    ...
    while (state == 'active'):
        ...
        if self_measure_received == 'True':
            ...
            # records UAVS self estimation values
        xhat_solo_str = str('%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f \n' % (
        xhat_Solo[0], xhat_Solo[1], xhat_Solo[2], xhat_Solo[3], xhat_Solo[4], xhat_Solo[5], xhat_Solo[6], xhat_Solo[7],
        xhat_Solo[8], xhat_Solo[9], xhat_Solo[10], xhat_Solo[11]))
        f = open('xhat_solo.txt', 'a')
        f.write(xhat_solo_str)
        # print 'test 3'
